...
@export var timeWhenFocused := 0.8
@export var focusTimeRamp := 2.0
@export var timeWhenRage := 1.25
@export var rageTimeRamp := 2.0
...
func _process(_delta: float):
	if Input.is_action_pressed("rage"):
		rageFocus -= 2.0 * _delta;
	if Input.is_action_pressed("focus"):
		rageFocus += 2.0 * _delta;
	rageFocus = clamp(rageFocus, -1.0, 1.0)
	if (rageFocus > 0):
		Engine.time_scale = 1 - (rageFocus ** focusTimeRamp)*(1 - timeWhenFocused)
	else:
		Engine.time_scale = 1 + (rageFocus ** focusTimeRamp)*(timeWhenRage - 1)
	displayProperties()
...
func displayProperties():
	...
	if hud != null:
		...
		hud.displayRageFocus(rageFocus)
		hud.displayTimeDelta(Engine.time_scale)
