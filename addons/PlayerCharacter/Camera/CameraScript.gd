...
@export var rageDelta : float
@export var focusDelta : float
...
func applies(delta : float):
	...
	if playChar.stateMachine.currStateName == "Crouch":
		...
		camera.fov = lerp(camera.fov, runFOV + rfMod, fovTransitionSpeed * delta)
		...
		camera.fov = lerp(camera.fov, startFOV + rfMod, fovTransitionSpeed * delta)
		
func getFovModifier():
	var rf = playChar.rageFocus
	if (rf < 0):
		return rageDelta * rf
	else:
		return focusDelta * rf
